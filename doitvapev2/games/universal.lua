...
vape.Libraries.string = loadstring(downloadFile('doitvapev2/libraries/string.lua'), 'string')()
...
local hash = loadstring(downloadFile('doitvapev2/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('doitvapev2/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('doitvapev2/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
run(function()
	...
	function whitelist:announce(text)
		...
		icon.Image = getcustomasset('doitvapev2/assets/new/vape.png')
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('doitvapev2/profiles/whitelist.json') and readfile('doitvapev2/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('doitvapev2/profiles/whitelist.json', whitelist.textdata)
				end)
			end
...
run(function()
	...
	Radar = vape:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('doitvapev2/assets/new/radaricon.png'),
...
run(function()
    ...
    local function Added(ent)
    	...
    	arrow.Visible = false
    	arrow.Image = getcustomasset('doitvapev2/assets/new/arrowmodule.png')
    ...
    local ESPAdded = {
    	Drawing2D = function(ent)
    		...
    		if Name.Enabled then
    			...
    			local playerTags = ""
    			if shared.WhitelistData and shared.WhitelistData.tags then
    				for _, tag in ipairs(shared.WhitelistData.tags) do
    					playerTags = playerTags .. string.format("[%s] ", tag.text)
    				end
    			end
    			EntityESP.Drop.Text = ent.Player
    					and playerTags .. whitelist:tag(ent.Player, true) .. (DisplayName.Enabled and ent.Player.DisplayName or ent.Player.Name)
...
run(function()
    ...
    local Added = {
    	Normal = function(ent)
    		...
    		if shared.WhitelistData and shared.WhitelistData.tags then
    			for _, tag in ipairs(shared.WhitelistData.tags) do
    				local color = tag.color or {255, 255, 255}
    				playerTags = playerTags .. string.format('<font color="rgb(%d, %d, %d)">[%s]</font> ', color[1], color[2], color[3], tag.text)
    			end
    		end
    		Strings[ent] = ent.Player
    				and playerTags .. whitelist:tag(ent.Player, true, true) .. (DisplayName.Enabled and ent.Player.DisplayName or ent.Player.Name)
    			or ent.Character.Name
