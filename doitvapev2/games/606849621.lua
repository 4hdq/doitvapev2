...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/4hdq/doitvapev2/'..readfile('doitvapev2/profiles/commit.txt')..'/'..select(1, path:gsub('doitvapev2/', '')), true) end)
...
local vm = loadstring(downloadFile('doitvapev2/libraries/vm.lua'), 'vm')()
